...
%% NMM Signal Waveform
random_samples = randi([0,nper-1],994*n_iter*4,n_sources);                 % the waveform index for each source
...
load(['../source/nmm_clip/a' int2str(region_id(1)-1) '/nmm_' int2str(nmm_idx) '.mat'])
...
v = b-a;
v = v./norm(v,2);  % Changed from mynorm(v,2)
end
